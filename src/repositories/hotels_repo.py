...
class HotelsRepo(BaseRepository):
    ...
    async def get_available(
        ...
        available_rooms_at_hotel = get_available_by_date(date_from, date_to).cte(
            "available_rooms_at_hotel"
        )

        query = (
            select(
                hotels,
                func.sum(available_rooms_at_hotel.c.available_rooms).label(
                    "available_rooms"
                ),
            ...
            .join(
                available_rooms_at_hotel,
                hotels.c.id == available_rooms_at_hotel.c.hotel_id,
            )
            .group_by(hotels)
            .limit(limit)
            .offset(offset)
        ...
        return [
            AvailableHotelMapper.map_to_domain_entity(hotel)
            for hotel in query_result.all()
        ]

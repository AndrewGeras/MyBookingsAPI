...
def get_available_by_date(date_from: date, date_to: date, hotel_id: int | None = None):

    booked_rooms_cte = (
        select(BookingsORM.room_id, func.count().label("rooms_count"))
    ...
    query = select(available_rooms_cte).filter(
        available_rooms_cte.c.available_rooms > 0,

...
async def register_user(
    db: DBDep, user: Annotated[UserRequestAdd, Body(openapi_examples=user_example)]
):
...
@router.post("/login", description="<h2>Ручка для авторизации пользователя</h2>")
async def login(db: DBDep, user_data: UserAuth, response: Response):

...
async def get_hotels(
    ...
    hotels = await db.hotels.get_all(
        title=title, location=location, limit=per_page, offset=per_page * (page - 1)
    )
    return hotels
...
async def get_available_hotels(
    ...
    date_from: date = Query(
        description="дата заезда", default=df_exmp, examples=[df_exmp]
    ),
    date_to: date = Query(
        description="дата выезда", default=dt_exmp, examples=[dt_exmp]
    ),
    title: str | None = Query(default=None, description="название отеля"),
    location: str | None = Query(default=None, description="локация отеля"),
):
    ...
    hotels = await db.hotels.get_available(
        title=title,
        location=location,
        limit=per_page,
...
@router.get(
    "/{hotel_id}", description="<h2>Ручка для получения одного отеля по его ID</h2>"
)
...
async def create_hotel(
    db: DBDep, hotel: Annotated[HotelAdd, Body(openapi_examples=hotel_example)]
):

...
async def get_all_rooms(
    ...
    rooms = await db.rooms.get_available(
...
async def update_rooms(
    db: DBDep, hotel_id: int, room_id: int, rooms_data: RoomAddRequest
):
    await db.rooms.edit(
        data=RoomAdd(hotel_id=hotel_id, **rooms_data.model_dump()), id=room_id
    )
    await db.room_facilities.upsert_or_delete(
        room_id=room_id, facilities_ids=rooms_data.facilities_ids
    )
    await db.commit()
...
async def edit_rooms(
    ...
    await db.rooms.edit(
        RoomPatch(hotel_id=hotel_id, **rooms_data.model_dump(exclude_none=True)),
        exclude_unset=True,
        id=room_id,
    )
    await db.room_facilities.upsert_or_delete(
